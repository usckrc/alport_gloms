---
title: "475 Ambient RNA and Doublet Removal"
...
1) I'm not sure I can explain this, but there is some strange processing going on between the raw and filtered files and they don't have the same number of genes in them...so I would error out on the `sc = SoupChannel(filtered_matrix,toc)` code. To fix this, I filtered the tod to have the same genes as the toc with `rows_to_keep <- toc@Dimnames[[1]]` and `filtered_matrix <- tod[rows_to_keep, , drop = FALSE]` <br>

2) Needed to adjust `sc = autoEstCont(sc, tfidfMin = .5)`

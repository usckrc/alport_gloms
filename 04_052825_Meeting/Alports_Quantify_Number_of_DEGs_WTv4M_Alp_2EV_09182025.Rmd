---
title: "Alports Quantifying Number of DEGs by Cell Type: 4M_Alp_2EV vs. WT"
...
SO2 <- subset(SO, subset = group == "4M_Alp_2EV")
...
for (i in clusters) {
DefaultAssay(SO) <- "SCT"
DEGs <- FindMarkers(SO, ident.1 = "4M_Alp_2EV", ident.2 = "WT", group.by = "group" , subset.ident = i, logfc.threshold = .1, min.pct = .1)
...
df <- df %>%
  kbl(caption = paste("Top 20", i, "Genes Upregulated in 4M_Alp_2EV")) %>%
...
save(list = list_objects, file = here("outputs", "Alports_4M_Condition_DEGs_4M_Alp_2EV_v_WT.Rdata"), envir = DEG.env)
...
write.xlsx(alldf, sheetName = names(alldf), file = here("outputs", "Alports_4M_Condition_DEGs_4M_Alp_2EV_v_WT.xlsx"))
...
load(here("outputs", "Alports_4M_Condition_DEGs_4M_Alp_2EV_v_WT.Rdata"))
...
p1 <- ggplot(output_table2, aes(x = Data_Frame)) +
  ...
  ggtitle("Cell Type DEGs 4M_Alp_2EV vs. WT") +
